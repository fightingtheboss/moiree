...
class SummariesController < ApplicationController
  def show
    @edition = Edition.includes(:categories, :critics, :selections, ratings: :critic).friendly.find(params[:edition_id])
    ...
    @most_divisive = @edition.selections.includes(:ratings).sort_by(&:ratings_standard_deviation).reverse.first
    @most_divisive_histogram = @most_divisive.ratings.group_by(&:score).transform_values(&:size)
    (0..5).step(0.5).each do |score|

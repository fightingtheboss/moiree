...
**Stack:** Ruby 4.0.4 (REQUIRED) | Rails 8.0.2 | SQLite3 | Turbo + Tailwind v4 | Solid Queue (background jobs)
...
**Prerequisites:** Ruby 4.0.4, bundler, SQLite3, libvips libs. Use `ruby/setup-ruby@v1` in CI or `.devcontainer/`.

```bash
# 1. Install dependencies
bundle install
# Error: "Ruby version 3.x.x but Gemfile specified 4.0.4"? Install Ruby 4.0.4 or use devcontainer.
...
2. Ruby 4.0.4 + gems via `ruby/setup-ruby@v1` (auto-caches)
...
**1. Ruby version mismatch:** Install Ruby 4.0.4 or use `.devcontainer/`
...
```
.devcontainer/         → Docker devcontainer with Ruby 4.0.4
...
Gemfile               → Ruby 4.0.4, Rails 8.0.2, solid_queue, tailwindcss-rails, kamal, litestream, etc.
